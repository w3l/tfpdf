...
function GetStringWidth($s)
{
	...
	if ($this->unifontSubset) {
		...
		foreach($unicode as $char) {
			if (isset($cw[$char])) { $w += (@ord($cw[2*$char])<<8) + @ord($cw[2*$char+1]); } // Quick and dirty "fix" for: Uninitialized string offset(got 2 warnings after changing the code below)
...
function _putTTfontwidths(&$font, $maxUni) {
	...
	//$cwlen = $maxUni + 1; 
    $cwlen = min(($maxUni + 1), 65535); // Fix for https://github.com/rev42/tfpdf/issues/7
